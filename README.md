...
- A REG_NGP restarts the handshake at REG1 only when nothing is in flight anywhere: no uplink is awaiting a REG2 or a REG3, and the receiving uplink is not itself already registered. The uplink's own connected state is checked directly, because the aggregate active-connection count is refreshed only once per housekeeping tick and is briefly stale for a link that just completed REG3 — long enough for a forged REG_NGP to re-open the window in which a REG_ERR would be honored.
