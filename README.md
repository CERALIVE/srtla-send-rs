...
- A successful socket reconnect keeps using its cached receiver peer and never waits on diagnostic DNS. Re-resolution runs in a detached task with a 3-second bound, at most one process-wide lookup at a time, empty answers treated as inconclusive, and at most one drift warning per minute across the bond. It never repoints one uplink independently; coordinated whole-bond receiver migration remains deferred.
