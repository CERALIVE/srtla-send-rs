...
**Superseded by [ADR-004 (control dialect)](ADR-004-control-dialect.md) for the
transport.** Everything below that describes the control-socket *wire shape* is
**withdrawn**: the `hello` handshake, `get-capabilities`, `subscribe-events` with
`"event"` notifications, the kebab-case method names, the
`/tmp/srtla-send-control-<port>.sock` path convention, the last-known-snapshot replay
on subscribe, and the "dual-support" text-protocol routing. The sender now speaks upstream `irlserver/srtla_send`'s
JSON-RPC 2.0 dialect verbatim (snake_case `get_status` / `get_stats` / `subscribe` /
`unsubscribe`, no handshake, no replay) and extends it only additively
(`get_capabilities`, `--capabilities-json`, optional telemetry fields). ADR-004 is the
normative record for all of that.

What remains **in force** from this document is the telemetry *document* contract: the
`--stats-file` snapshot shape, its field names, types, and units (`bitrate_bps` = wire
bytes/s × 8), the `connections: []` idle-not-absent rule, and the atomic publish. That
contract is implemented in `src/telemetry_doc.rs` (schema, units, serializer) and
`src/telemetry_file.rs` (temp sibling → `fsync` → `rename(2)`), and is extended by
[ADR-002](ADR-002-session-bytes-telemetry.md) (cumulative `bytes_sent_total`) and
[ADR-003](ADR-003-bind-map-contract.md) (the four optional identity/mode echoes).

The original text is retained unchanged below as schema history. Code paths it names
(`src/config.rs` text protocol, `DynamicConfig` setters, `set-mode` / `set-rtt-delta`,
the `rtt-threshold` mode) belong to the pre-hard-fork tree and do not exist on this base.

---

Accepted (historical; transport section superseded, see above)
