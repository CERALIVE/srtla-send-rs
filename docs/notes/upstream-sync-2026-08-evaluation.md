...
| 86b90aa | 2026-07-23 | feat(srtla_send): bind uplinks by interface index on apple targets | ADOPT | optional low-priority, `cfg(apple)`-gated, zero device (Linux aarch64) impact, eases future merges |
...
| b909220 | 2026-07-15 | fix(network-sim): drain child pipes so the process under test cannot wedge | REJECT | harness robustness fix for upstream's own `network-sim` child-process design; not confirmed needed against our differently-implemented `crates/network-sim` |
