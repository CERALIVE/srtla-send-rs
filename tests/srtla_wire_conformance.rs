//! BELABOX SRTLA wire-conformance golden tests (T16).
//!
//! These tests LOCK byte-level compatibility between this Rust sender and the
//! canonical BELABOX C reference, pinned at commit
//! `37862da3d0c13b46956efd3f88877053293d97d6`:
//!
//! - Type codes + `SRTLA_ID_LEN` + REG frame sizes — `common.h` lines 26-40
//!   <https://github.com/BELABOX/srtla/blob/37862da3d0c13b46956efd3f88877053293d97d6/common.h#L26-L40>
//! - REG1/REG2/REG3 build (`htobe16(type)` header + 256-byte id) — `srtla_rec.c`
//!   `group_reg`/`conn_reg`
//!   <https://github.com/BELABOX/srtla/blob/37862da3d0c13b46956efd3f88877053293d97d6/srtla_rec.c#L255-L320>
//! - ACK layout `struct { uint32_t type; uint32_t acks[10]; }` and
//!   `ack.type = htobe32(SRTLA_TYPE_ACK << 16)` — `srtla_rec.c` `register_packet`
//!   <https://github.com/BELABOX/srtla/blob/37862da3d0c13b46956efd3f88877053293d97d6/srtla_rec.c#L389-L395>
//!
...
//! has changed and the sender is no longer interoperable with a BELABOX receiver;
...
/// `RECV_ACK_INT` in the BELABOX receiver (`srtla_rec.c`): the fixed number of
/// per-connection sequence numbers carried in one SRTLA ACK frame.
const BELABOX_RECV_ACK_INT: usize = 10;

/// `sizeof(srtla_ack_pkt)` = `sizeof(u32 type) + sizeof(u32 acks[10])` = 44.
const BELABOX_ACK_PKT_LEN: usize = 4 + 4 * BELABOX_RECV_ACK_INT;

// ---------------------------------------------------------------------------
// 1. Type codes — exact hex, big-endian wire order (common.h #L26-L40)
// ---------------------------------------------------------------------------

#[test]
fn type_codes_match_belabox_common_h() {
    // SRTLA control-plane type codes — verbatim from common.h.
...
fn type_codes_serialize_big_endian_on_the_wire() {
    // The C reference sends headers via `htobe16(type)`; our builders use
...
// ---------------------------------------------------------------------------
// 2. Sizes — SRTLA_ID_LEN and REG frame lengths (common.h #L36-L40)
// ---------------------------------------------------------------------------
...
fn ack_layout_is_44_bytes_type_plus_ten_acks() {
    ...
    assert_eq!(BELABOX_RECV_ACK_INT, 10);
    assert_eq!(
        BELABOX_ACK_PKT_LEN, 44,
        "ACK struct = 4 (type) + 40 (10x u32 acks) = 44"
    );
}

// ---------------------------------------------------------------------------
// 3. Builders produce BELABOX-exact wire bytes
// ---------------------------------------------------------------------------

#[test]
fn reg1_builder_matches_belabox_wire_layout() {
...
#[test]
fn reg2_builder_matches_belabox_wire_layout() {
    let mut id = [0u8; SRTLA_ID_LEN];
    for (i, b) in id.iter_mut().enumerate() {
        *b = (255 - (i & 0xff)) as u8;
    }
...
#[test]
fn ack_builder_matches_belabox_wire_layout() {
    // BELABOX: ack.type = htobe32(SRTLA_TYPE_ACK << 16) = 0x9100_0000
...
// ---------------------------------------------------------------------------
// 4. Parser reads a BELABOX-shaped ACK
// ---------------------------------------------------------------------------

#[test]
fn parser_reads_belabox_shaped_ack() {
    ...
    let mut frame = [0u8; BELABOX_ACK_PKT_LEN];
    ...
    assert_eq!(
        parsed.as_slice(),
        &seqs[..],
        "parser must recover all 10 BELABOX ACK seqs in order"
    );
}
