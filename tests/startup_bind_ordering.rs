...
//!
//! CeraUI spawns `srtla_send` and then immediately tells the encoder to open an
//! SRT connection to the local listen port, with no readiness handshake in
//! between. If the listener is not bound by then the encoder's handshake goes
//! unanswered and the operator sees a hard `SRT_REJ_TIMEOUT` stream-start
//! failure. These tests pin the ordering that closes that window: the local SRT
//! listener is bound before any uplink is dialed, so the wait for uplink setup
//! (a sequential per-link resolve + bind + connect) can never delay it.
//!
...
impl SenderProc {
    fn spawn(args: &[&str]) -> Self {
        ...
        // preserves their relative order in the buffer. stderr is pumped into the
        // same buffer only so a panic/anyhow message shows up in assertion output.
