//! Edge-case hardening for the ADR-001 sender telemetry serializer.
//!
...
//! `build_current_telemetry_json` reads the real wall clock. Every assertion
//! below feeds a fixed timestamp through that seam, so the suite is reproducible
//! and never flaky on `last_updated_ms`. They complement the in-module unit
//! tests in `src/telemetry_doc.rs` and `src/telemetry_file.rs`.
...
/// A legacy-shaped snapshot document, scoped to the ADR-001 contract.
...
fn schema_version_is_pinned_to_one() {
    // Guard the on-wire `schema_version` against an accidental change: the
    // consumer validates a literal 1, so any drift silently breaks every
    // reader. Change this assertion ONLY alongside a versioned telemetry
