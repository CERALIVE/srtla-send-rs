...
fn a_reload_removes_and_re_adds_a_twin_under_a_stable_link_id() {
    ...
    let after_removal = stack
        .wait_for_log("removed 1 stale connections", TICK_DETECTED)
    ...
    let after_readd = stack
        .wait_for_log("added 1 new connections", TICK_DETECTED)
...
fn a_degraded_reload_retains_the_mapped_twin_pool() {
    ...
    assert!(
        !log.contains("removed 1 stale connections")
            && !log.contains("removed 2 stale connections"),
        "a degraded reload must not tear a live bond down:\n{log}"
    );

    // And: both twins are still on the wire afterwards.
    sleep(Duration::from_secs(5));
    for (idx, start) in before.iter().enumerate() {
        let carried = stack.topo.tx_bytes(idx).expect("read tx_bytes") - start;
        assert!(
            carried > 100,
...
fn a_file_order_swap_keeps_each_link_id_on_its_own_interface() {
    ...
    assert!(
        !reloaded.contains("stale connections") && !reloaded.contains("new connections"),
    ...
    for (idx, spoken) in spoken_before.iter().enumerate() {
        assert!(
            count_lines_naming(&after, &stack.label(idx)) > *spoken,
...
fn a_reordered_sidecar_at_the_same_generation_is_refused() {
    ...
    assert!(
        !log.contains("new connections"),
        "an unorderable mapping must not be applied:\n{log}"
    );
    for row in &crossed {
        let moved = format!("on {} [{}]", row.iface, row.link_id);
        assert!(
            !log.contains(&moved),
...
fn a_route_removal_blackhole_is_reported_and_never_reads_healthy() {
    ...
    let status = stack
        .wait_for_log(
            &format!("iface={} route=no_default_route", stack.rows()[1].iface),
            STATUS_LOGGED,
        )
        .expect("the status log must report the blackholed link honestly");
    assert!(
        !status.contains(&format!(
            "iface={} route=no_default_route",
            stack.rows()[0].iface
        )),
