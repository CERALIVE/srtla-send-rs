...
use crate::bind_map::{EffectiveLink, IfaceName, LinkId};
...
impl UplinkSpec {
    ...
    pub fn label(&self, host: &str, port: u16) -> String {
        match (self.iface.as_ref(), self.link_id.as_ref()) {
            (None, _) => format!("{host}:{port} via {}", self.ip),
            (Some(iface), None) => {
                format!("{host}:{port} via {} on {}", self.ip, iface.as_str())
            }
...
impl From<&EffectiveLink> for UplinkSpec {
    fn from(link: &EffectiveLink) -> Self {
        Self {
            ip: link.ip,
