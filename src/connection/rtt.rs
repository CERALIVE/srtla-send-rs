    ...
    pub fn handle_keepalive_response(&mut self, data: &[u8], label: &str) -> Option<u64> {
        ...
        if let Some(ts) = extract_keepalive_timestamp(data) {
            ...
            if let Some(rtt) = self.record_round_trip(ts, now) {
                self.waiting_for_keepalive_response = false;
                debug!(
                    "{}: RTT from keepalive: {}ms (kalman: {:.1}ms, velocity: {:.2}ms/sample, jitter: \
                     {:.1}ms)",
                    label,
                    rtt,
...
mod tests {
    ...
    fn test_keepalive_zero_rtt_rejected() {
        // Regression for S7: a keepalive reply whose timestamp is >= now (same-ms
        // reply or future timestamp from clock skew) yields rtt == 0 via
        // saturating_sub. A 0ms RTT is not a real sample and must be rejected, or
        // it would initialize the filter and seed rtt_min_ms = 0, making the link
        // look artificially fast. Parity with the ACK path (ack_nak.rs).
        let mut tracker = RttTracker::default();
        assert!((tracker.rtt_min_ms - 200.0).abs() < f64::EPSILON);
