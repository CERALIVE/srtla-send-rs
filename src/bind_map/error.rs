//! Every way a bind-map pair can be refused, and the frozen reason it reports.
//!
//! Two levels on purpose. [`BindMapError`] is fine-grained so a log line tells a
//! writer exactly which row and field to fix; [`DegradedReason`] (defined once,
//! in [`super::status`]) is the small frozen set that leaves the process as
//! typed telemetry. The mapping between them is total and exhaustive — see
//! [`BindMapError::reason`].
