...
// Frozen on-wire byte pins mirroring the C contracts in
// srtla/tests/test_registration_handshake.cpp (REG1/REG2 = 258 B, REG3 = 2 B)
// and srtla/tests/test_extended_keepalive.cpp (bare KEEPALIVE = 2 B). A failure
// here means a layout/constant drifted and retro-compat with the receiver broke.
...
mod encode {
    ...
    fn keepalive_is_bare_2_bytes() {
        // Caveat that prevents a false "fix": this fork's live send_keepalive()
        // emits the backwards-compatible *extended* 38-byte KA, not this bare
        // form. This pins the minimal legacy 2-byte keepalive the protocol still
        // guarantees; it does not assert which form the sender emits.
