//! Keepalive interop conformance (T17).
//!
//! Pins the documented keepalive divergence between this fork and BELABOX, and
//! the defensive-parse contract that lets the two interoperate:
//!
//! - **BELABOX** (github.com/BELABOX/srtla, `srtla_send.c` L589-593) sends a
//!   *bare* 2-byte keepalive — the type only (`htobe16(SRTLA_TYPE_KEEPALIVE)`),
//!   no timestamp.
//! - **This fork** inherited irlserver's TIMESTAMPED keepalive: a standard 10 B
//!   frame (type + `u64` ms timestamp) and the backwards-compatible *extended*
//!   38 B frame (timestamp + a `0xC01F`-tagged `ConnectionInfo` telemetry
//!   trailer).
//!
//! These tests assert (a) our extended keepalive builds → parses → preserves
//! its RTT fields end-to-end through the real receive path, and (b)/(c) that a
//! bare 2-byte BELABOX-style echo — and any truncated/oversized frame — is
//! handled gracefully (no error, no panic). They do NOT change the wire format
//! and do NOT add a 2-byte-compat keepalive mode (deferred follow-up).
//!
//! See `docs/KEEPALIVE_INTEROP.md` for the full divergence write-up, the
//! receiver-echo interop ASSUMPTION, and the deferred 2-byte-compat note.
...
mod tests {
    ...
    /// (b) A bare 2-byte BELABOX-style keepalive echo is accepted without error
    /// or panic (defensive parse).
    ///
    /// BELABOX receivers may echo a bare `[0x90, 0x00]` keepalive (type only,
    /// no timestamp). Our receive path must tolerate it: it is recognised as a
    /// keepalive, yields no timestamp/telemetry (too short), and the RTT path
    /// returns `None` cleanly instead of panicking. NOTE: with no timestamp no
    /// RTT can be measured off a bare echo — see `docs/KEEPALIVE_INTEROP.md`.
    #[test]
    fn keepalive_bare_2byte_accepted() {
        // Exactly what BELABOX srtla_send.c sends: htobe16(SRTLA_TYPE_KEEPALIVE).
