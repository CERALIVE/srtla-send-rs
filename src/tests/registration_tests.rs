...
mod tests {
    ...
    // Ports srtla test_registration_handshake.cpp: two-phase SRTLA v2 flow, driven
    // from the sender side — REG1 -> REG2(full_id) -> REG2 broadcast -> REG3.
    #[tokio::test]
    ...
    // Ports srtla Reg2ReplyEchoesClientIdInFullId: first SRTLA_ID_LEN/2 bytes echo
    // the sender id, the tail is receiver-substituted.
    #[test]
    ...
    // Ports srtla test_sender_bootstrap.cpp NeverReceivedIsNotTimedOut: a fresh link
    // (last_received == None, not yet connected, within startup grace) drives
    // registration, never reconnection — even after the virtual clock passes
    // CONN_TIMEOUT. is_timed_out() reads tokio::time::Instant, honoring the paused clock.
    #[tokio::test(start_paused = true)]
