...
//!
//! The version line is an operator-visible surface: CeraUI shells out to
//! `srtla_send -v` and renders the raw stdout in its Settings → Versions dialog.
//! So the string has to stay honest when the build carried no git context —
//! a shipped binary once read
//!
//! ```text
...
//! uncommitted changes, none of which are true. `build.rs` now emits empty
//! strings for anything it could not resolve and [`compose_version_line`] omits
//! the parenthetical entirely, matching the convention of every other CLI that
