...
pub struct BitrateTracker {
    ...
    ///
    /// **Monotonic.** [`BitrateTracker::reset`] deliberately does NOT clear it:
    /// a socket replacement (`SrtlaConnection::reset_for_reconnect`) is a
    /// transient link event, not a new streaming session, and a total that
    /// restarts on every radio blip is worthless. Only a fresh process starts
    /// it at 0.
    pub bytes_sent_total: u64,
    /// `bytes_sent_total` as of the last rate calculation — the baseline the
    /// 2-second window subtracts. [`BitrateTracker::reset`] rebases this to
    /// `bytes_sent_total` (not 0), so the first post-reset window still
    /// measures a zero delta.
...
impl BitrateTracker {
    ...
    /// (rather than zeroing both) keeps the next [`BitrateTracker::calculate`]
    /// delta at 0 — byte-identical rate behavior to zeroing both — while the
    /// cumulative counter survives the reconnect (ADR-002).
...
mod tests {
    ...
    // ---- ADR-002: the cumulative total survives a socket replacement -------
